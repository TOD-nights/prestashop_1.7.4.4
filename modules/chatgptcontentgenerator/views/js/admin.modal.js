...
var ChatGptModal = (function () {
    ...
    ChatGptModal.prototype.render = function() {
        ...
        var obj = $('<div class="modal fade ' + options.class + '" id="' + options.id + '" tabindex="-1" />');
        $('body').append(obj);
        obj.html(
        ...
        if (!!options.actions) {
            var actionsWrapper = $('#' + options.id + ' .modal-footer');
            for (var i = 0; i < options.actions.length; i++) {
                options.actions[i].renderInto(actionsWrapper);
            }
        }
    ...
    ChatGptModal.prototype.open = function() {
        if (this.getOptions().isRendered == false) {
            this.render();
        }

        $('#' + this.getOptions().id).modal('show');
        return this;
    };
